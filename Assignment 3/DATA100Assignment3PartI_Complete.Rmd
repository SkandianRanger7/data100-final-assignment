...
cyclones_data <- cyclones_data_update_2 |>
    mutate(
        category = ordered(
            case_when(
                max_wind <= 33 ~ "TD",
                34 <= max_wind & max_wind <= 63 ~ "TS",
                64 <= max_wind & max_wind <= 82 ~ "1",
                83 <= max_wind & max_wind <= 95 ~ "2",
                96 <= max_wind & max_wind <= 112 ~ "3",
                113 <= max_wind & max_wind <= 136 ~ "4",
                137 <= max_wind ~ "5"
            ),
            levels = cat_levels
        )
...
covid_2020 <- covid_cases |>
  ...
  group_by(location) |>
  summarise(continent = continent[1], total_cases = sum(total_cases)) |>
